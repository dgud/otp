...
array_with_list(N, Type, ListAcc, ArrAcc) ->
    ...
    oneof([ %% Set/append/prepend many at end/beginning
            ?LET(List, list(Type),
                 {ListAcc ++ List, lists:foldl(fun array:append/2, ArrAcc, List)}),
            ?LET(List, list(Type),
            ...
            %% Resize, shift
            ?LET(I, small_nat(), array_list_resize(I, ListAcc, ArrAcc)),
            ?LET(I, int(), array_list_shift(I, ListAcc, ArrAcc))
          ]).

array_list_set(I, V, L0, A) ->
    Size = array:size(A),
    if I < Size ->
            {L1, [_|L2]} = lists:split(I, L0),
            {L1 ++ [V|L2], array:set(I, V, A)};
       I == Size ->
            {L0 ++ [V], array:set(I, V, A)};
       true ->
            Def = array:default(A),
            Idx = I rem (Size+17),  %% Allow up to next bucket
            L2 = lists:duplicate(Idx-Size, Def) ++ [V],
            {L0++L2, array:set(Idx, V, A)}
    end.

array_list_reset(I, L0, A) ->
    Size = array:size(A),
    if I < Size ->
            Def = array:default(A),
            {L1, [_|L2]} = lists:split(I, L0),
            {L1 ++ [Def|L2], array:reset(I, A)};
       true ->
            {L0, array:reset(I, A)}
    end.

array_list_resize(NewSz0, L0, A) ->
    Size = array:size(A),
    if NewSz0 < Size ->
            {L1, _L2} = lists:split(NewSz0, L0),
            {L1, array:resize(NewSz0, A)};
       true ->
            %% Limit NewSize
            Extend = (NewSz0 rem (Size+17)),
            Def = array:default(A),
            L1 = lists:duplicate(Extend, Def),
            {L0 ++ L1, array:resize(Size+Extend, A)}
    end.

array_list_shift(I0, L0, A) ->
    Size = array:size(A),
    Steps = if Size =:= 0 ->
                    -abs(I0 rem 10);
               true ->
    ...
    L1 = case Steps >= 0 of
             ...
             false ->
                 lists:duplicate(abs(Steps), array:default(A)) ++ L0
         end,
    {L1, array:shift(Steps, A)}.
