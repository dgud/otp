...
map(Function, Array) when is_function(Function, 2) ->
    {Array1, _} = mapfoldl(fun (I, V, _) -> {Function(I, V), []} end,
                           [], Array),
    Array1;
map(_, _) ->
    erlang:error(badarg).
...
sparse_map(Function, Array) when is_function(Function, 2) ->
    {Array1, _} = sparse_mapfoldl(fun (I, V, _) -> {Function(I, V), []} end,
                                  [], Array),
    Array1;
sparse_map(_, _) ->
    erlang:error(badarg).
...
-spec foldl_3(array_indx(), array_indx(), array_indx(), _,
...
sparse_foldl(Low, High, Function, InitialAcc, #array{default = D}=Array)
  when is_function(Function, 3) ->
    foldl(Low, High, fun (_I, V, A) when V =:= D -> A;
                         (I, V, A) -> Function(I, V, A)
                     end, InitialAcc, Array);
...
sparse_foldr(Low, High, Function, InitialAcc, #array{default = D}=Array)
  when is_function(Function, 3) ->
    foldr(Low, High, fun (_I, V, A) when V =:= D -> A;
                         (I, V, A) -> Function(I, V, A)
                     end, InitialAcc, Array);
...
""".
-spec sparse_mapfoldl(Function, InitialAcc :: A, Array :: array(Type)) -> A when
      Function :: fun((Index :: array_indx(), Value :: Type, Acc :: A) -> A).
...
""".
-spec sparse_mapfoldr(Function, InitialAcc :: A, Array :: array(Type)) -> A when
      Function :: fun((Index :: array_indx(), Value :: Type, Acc :: A) -> A).
