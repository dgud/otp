...
setDockSizeConstraint(#wx_ref{type=ThisT}=This,Width_pct,Height_pct)
 when is_number(Width_pct),is_number(Height_pct) ->
  ?CLASS(ThisT,wxAuiManager),
  wxe_util:queue_cmd(This,Width_pct,Height_pct,?get_env(),?wxAuiManager_SetDockSizeConstraint).
