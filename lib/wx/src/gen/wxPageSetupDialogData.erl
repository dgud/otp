...
new(#wx_ref{type=PrintDataT}=PrintData) ->
  ...
  wxe_util:queue_cmd(PrintData,?get_env(),PrintDataOP),
  wxe_util:rec(PrintDataOP).
...
setPaperSize(#wx_ref{type=ThisT}=This,Id)
 when is_integer(Id) ->
  ?CLASS(ThisT,wxPageSetupDialogData),
  wxe_util:queue_cmd(This,Id,?get_env(),?wxPageSetupDialogData_SetPaperSize_1_0);
setPaperSize(#wx_ref{type=ThisT}=This,{SzW,SzH} = Sz)
 when is_integer(SzW),is_integer(SzH) ->
  ?CLASS(ThisT,wxPageSetupDialogData),
  wxe_util:queue_cmd(This,Sz,?get_env(),?wxPageSetupDialogData_SetPaperSize_1_1).
