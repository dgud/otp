...
new() ->
  wxe_util:queue_cmd(?get_env(), ?wxSizerItem_new_0),
  wxe_util:rec(?wxSizerItem_new_0).
...
new(#wx_ref{type=WindowT}=Window,#wx_ref{type=FlagsT}=Flags) ->
  ...
  wxe_util:queue_cmd(Window,Flags,?get_env(),WindowOP),
...
new(Width,Height,#wx_ref{type=FlagsT}=Flags)
 when is_integer(Width),is_integer(Height) ->
  ?CLASS(FlagsT,wxSizerFlags),
  wxe_util:queue_cmd(Width,Height,Flags,?get_env(),?wxSizerItem_new_3),
...
new(#wx_ref{type=WindowT}=Window,Proportion,Flag,Border,#wx_ref{type=UserDataT}=UserData)
 when is_integer(Proportion),is_integer(Flag),is_integer(Border) ->
  ...
  wxe_util:queue_cmd(Window,Proportion,Flag,Border,UserData,?get_env(),WindowOP),
  wxe_util:rec(WindowOP).
...
new(Width,Height,Proportion,Flag,Border,#wx_ref{type=UserDataT}=UserData)
 when is_integer(Width),is_integer(Height),is_integer(Proportion),is_integer(Flag),is_integer(Border) ->
  ?CLASS(UserDataT,wx),
  wxe_util:queue_cmd(Width,Height,Proportion,Flag,Border,UserData,?get_env(),?wxSizerItem_new_6),
  wxe_util:rec(?wxSizerItem_new_6).
...
setSizer(#wx_ref{type=ThisT}=This,#wx_ref{type=SizerT}=Sizer) ->
  ?CLASS(ThisT,wxSizerItem),
  ?CLASS(SizerT,wxSizer),
  wxe_util:queue_cmd(This,Sizer,?get_env(),?wxSizerItem_SetSizer).
...
-spec setSpacer(This, Size) -> 'ok' when
	This::wxSizerItem(), Size::{W::integer(), H::integer()}.
setSpacer(#wx_ref{type=ThisT}=This,{SizeW,SizeH} = Size)
 when is_integer(SizeW),is_integer(SizeH) ->
  ?CLASS(ThisT,wxSizerItem),
  wxe_util:queue_cmd(This,Size,?get_env(),?wxSizerItem_SetSpacer_1).

%% @doc See <a href="http://www.wxwidgets.org/manuals/2.8.12/wx_wxsizeritem.html#wxsizeritemsetspacer">external documentation</a>.
-spec setSpacer(This, Width, Height) -> 'ok' when
	This::wxSizerItem(), Width::integer(), Height::integer().
setSpacer(#wx_ref{type=ThisT}=This,Width,Height)
 when is_integer(Width),is_integer(Height) ->
  ?CLASS(ThisT,wxSizerItem),
  wxe_util:queue_cmd(This,Width,Height,?get_env(),?wxSizerItem_SetSpacer_2).
...
setWindow(#wx_ref{type=ThisT}=This,#wx_ref{type=WindowT}=Window) ->
  ?CLASS(ThisT,wxSizerItem),
  ?CLASS(WindowT,wxWindow),
  wxe_util:queue_cmd(This,Window,?get_env(),?wxSizerItem_SetWindow).
