...
blit(#wx_ref{type=ThisT}=This,{DestPtX,DestPtY} = DestPt,{SzW,SzH} = Sz,#wx_ref{type=SourceT}=Source,{SrcPtX,SrcPtY} = SrcPt, Options)
...
-spec computeScaleAndOrigin(This) -> 'ok' when
	This::wxDC().
computeScaleAndOrigin(#wx_ref{type=ThisT}=This) ->
  ?CLASS(ThisT,wxDC),
  wxe_util:queue_cmd(This,?get_env(),?wxDC_ComputeScaleAndOrigin).
...
drawArc(#wx_ref{type=ThisT}=This,{Pt1X,Pt1Y} = Pt1,{Pt2X,Pt2Y} = Pt2,{CentreX,CentreY} = Centre)
 when is_integer(Pt1X),is_integer(Pt1Y),is_integer(Pt2X),is_integer(Pt2Y),is_integer(CentreX),is_integer(CentreY) ->
  ?CLASS(ThisT,wxDC),
  wxe_util:queue_cmd(This,Pt1,Pt2,Centre,?get_env(),?wxDC_DrawArc).
...
drawEllipse(#wx_ref{type=ThisT}=This,{PtX,PtY} = Pt,{SzW,SzH} = Sz)
 when is_integer(PtX),is_integer(PtY),is_integer(SzW),is_integer(SzH) ->
  ?CLASS(ThisT,wxDC),
  wxe_util:queue_cmd(This,Pt,Sz,?get_env(),?wxDC_DrawEllipse_2).
...
drawRoundedRectangle(#wx_ref{type=ThisT}=This,{RX,RY,RW,RH} = R,Radius)
 when is_integer(RX),is_integer(RY),is_integer(RW),is_integer(RH),is_number(Radius) ->
  ?CLASS(ThisT,wxDC),
  wxe_util:queue_cmd(This,R,Radius,?get_env(),?wxDC_DrawRoundedRectangle_2).
...
-spec getPixel(This, Pt) -> Result when
	...
	This::wxDC(), Pt::{X::integer(), Y::integer()}.
getPixel(#wx_ref{type=ThisT}=This,{PtX,PtY} = Pt)
 when is_integer(PtX),is_integer(PtY) ->
  ?CLASS(ThisT,wxDC),
  wxe_util:queue_cmd(This,Pt,?get_env(),?wxDC_GetPixel),
...
-spec setClippingRegion(This, Region) -> 'ok' when
	This::wxDC(), Region::wxRegion:wxRegion();
      (This, Rect) -> 'ok' when
	This::wxDC(), Rect::{X::integer(), Y::integer(), W::integer(), H::integer()}.
setClippingRegion(#wx_ref{type=ThisT}=This,#wx_ref{type=RegionT}=Region) ->
  ?CLASS(ThisT,wxDC),
  ?CLASS(RegionT,wxRegion),
  wxe_util:queue_cmd(This,Region,?get_env(),?wxDC_SetClippingRegion_1_0);
setClippingRegion(#wx_ref{type=ThisT}=This,{RectX,RectY,RectW,RectH} = Rect)
 when is_integer(RectX),is_integer(RectY),is_integer(RectW),is_integer(RectH) ->
  ?CLASS(ThisT,wxDC),
  wxe_util:queue_cmd(This,Rect,?get_env(),?wxDC_SetClippingRegion_1_1).
...
setUserScale(#wx_ref{type=ThisT}=This,X,Y)
 when is_number(X),is_number(Y) ->
  ?CLASS(ThisT,wxDC),
  wxe_util:queue_cmd(This,X,Y,?get_env(),?wxDC_SetUserScale).
