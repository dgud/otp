...
new(#wx_ref{type=BitmapT}=Bitmap,PaletteIndex)
 when is_integer(PaletteIndex) ->
  ?CLASS(BitmapT,wxBitmap),
  wxe_util:queue_cmd(Bitmap,PaletteIndex,?get_env(),?wxMask_new_2_0),
...
create(#wx_ref{type=ThisT}=This,#wx_ref{type=BitmapT}=Bitmap,PaletteIndex)
 when is_integer(PaletteIndex) ->
  ?CLASS(ThisT,wxMask),
  ?CLASS(BitmapT,wxBitmap),
  wxe_util:queue_cmd(This,Bitmap,PaletteIndex,?get_env(),?wxMask_Create_2_0),
