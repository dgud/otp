...
{
 public:
 wxeMetaCommand(ErlNifPid self, int EvId, wxeMemEnv *me)
     : wxEvent(EvId, wxeEVT_META_COMMAND)
    {  caller = self; memenv = me; } ;
 wxeMetaCommand(const wxeMetaCommand& event)
     : wxEvent(event)
    {  caller = event.caller; memenv = event.memenv; };
...
class wxeFifo {
 public:
    ...
    int Add(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[], int, wxeMemEnv *);
